export const convertToMatrix = <T>(
  source: Array<T>,
  rows: number,
  cols: number,
): T[][] => {
...
export const getAroundPositions = <T>(
  ...
  const positions = directions.map(
    ([dRow, dCol]) => [row + dRow, col + dCol] as MatrixPosition,
  );
  return positions.filter(([_row, _col]) => isInside([_row, _col], source));
};

export const isInside = <T>(
  position: MatrixPosition,
  source: T[][],
): boolean => {
  const [row, col] = position;
